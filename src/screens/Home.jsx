...
const Home = () => {
  ...
  const loadData = async () => {
    let response = await fetch("http://localhost:5000/api/foodsdata", {
      method: "POST",
      headers: {
          ...
          <div className="carousel-inner" id='carousel'>
            <div className='carousel-caption' style={{ zIndex: '10' }}>
              <div className="d-flex justify-content-center">
                <input className="form-control me-2" type="search" placeholder="Search" aria-label="Search" value={search} onChange={searchHandle}/>
          ...
          foodCat.length !== 0 ? foodCat.map((data) => {
            return (<div className='row mb-3'>
              ...
              {foodItem.length !== 0 ? foodItem.filter((item) => (item.CategoryName === data.CategoryName) && (item.name.toLowerCase().includes(search.toLocaleLowerCase())))
                .map((filterItems) => {
                  return (
                    <div key={filterItems._id} className='col-12 col-md-6 col-lg-3 text-dark'>
                      <Card
